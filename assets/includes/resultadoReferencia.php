		...
		<section>
			...
			if ($resultado)
			{
				...
				while ($row = mysql_fetch_assoc($resultado)) 
				{
					...
					?>
						<?php echo '<td class="indentificador-referencia" alt="'.$row['ref_codigo'].'">'; 
							echo ($bdr+1);?>
						</td>
						...
						?>
						<td><?php echo strtoupper($row2['tar_nombre1'])." ".strtoupper($row2['tar_nombre2'])." ".strtoupper($row2['tar_apaterno'])." ".strtoupper($row2['tar_amaterno']);?></td>
						...
						<?php 
							$consulta2 = "SELECT * FROM tb_profesional WHERE pro_cedula='".$row['ref_cedula_profesional'] ."' AND pro_estado='A'";
							$resultado2 = mysql_query($consulta2,$conexion2);
							$row2 = mysql_fetch_array($resultado2);
						?>
