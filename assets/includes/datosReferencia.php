<?php
	...
	if($_POST['tipo_trans']=="guarda")
	{
		$consulta = "INSERT INTO tb_matriz_referencia "
					."(ref_codigo,ref_fecha,ref_cedula_paciente,ref_telefono,ref_cedula_profesional,ref_unidad_refiere,ref_especialidad_referida,ref_unidad_referida,ref_tipo_servicio,ref_cie10,ref_diagnostico,ref_condicion_diagnostico,ref_edad,ref_sexo,ref_estado_ref,ref_estado)"
					." VALUES(NULL,"
					."'".date("Y-m-d")."',"
	...
	else
	{
		if($_POST['tipo_trans']=="mesReferencia")
		{
			$consulta = "SELECT * FROM tb_matriz_referencia "
				...
				."AND ref_fecha LIKE '".$_POST['ref_anio']."-%'";
			$resultado = mysql_query($consulta,$conexion);
			if(!$resultado)
			{
				$resultado_ga = array(
		...
		else
		{
			...
			{
				$consulta = "SELECT * FROM tb_matriz_referencia "
					."WHERE ref_fecha LIKE '".$_POST['ref_anio']."-%'";
				$resultado = mysql_query($consulta,$conexion);
				if(!$resultado)
				{
					$resultado_ga = array(
